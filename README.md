...
<span>$$\psi^{real}_{n,l,m}(r,\theta,\phi)=\begin{cases}\sqrt{2}(-1)^{m}Im(\psi_{n,l,|m|}(r,\theta,\phi)) & \text{if } m < 0\\\psi_{n,l,|m|}(r,\theta,\phi) & \text{if } m = 0 \\\sqrt{2}(-1)^{m}Re(\psi_{n,l,|m|}(r,\theta,\phi)) & \text{if } m > 0\end{cases}$$</span>
